...
export function ThreeDDropzone({ onFileSelect }: ThreeDDropzoneProps) {
    ...
    <div className="w-full flex flex-col justify-center items-start gap-4 sm:gap-6 md:gap-8">
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="text-sm md:text-base font-medium text-[var(--foreground)]">
              Drop a 3D model here, or choose file
            </div>
            <div className="text-xs font-mono text-[var(--muted-foreground)] mt-0.5">
              GLB · glTF · Wavefront OBJ · STL (Binary/ASCII) · PLY · 3MF · DAE · 3DS
            </div>
          </div>
        </div>
        ...
        <input
          ref={fileInputRef}
          type="file"
          accept=".stl,.obj,.glb,.gltf,.ply,.3ds,.dae,.3mf,.amf,.fbx"
          className="hidden"
