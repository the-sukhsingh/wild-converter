...
import type { ThreeDConversionResult } from "@/lib/three-d-converter";

interface ThreeDActionBarProps {
  isConverting: boolean;
  progressText?: string;
  result: ThreeDConversionResult | null;
  onConvert: () => void;
  onReset: () => void;
  disabled?: boolean;
}
    ...
    <div className="w-full pt-4 border-t border-[var(--border)] space-y-3">
      {result ? (
        <div className="space-y-3">
          {/* Summary for converted 3D output */}
            ...
            <div className="flex items-center gap-2 min-w-0 w-full sm:w-auto">
              ...
              <div className="min-w-0">
                ...
                <p className="text-[11px] font-mono text-[var(--muted-foreground)]">
                  {formatBytes(result.fileSizeBytes)} • {result.faceCount.toLocaleString()} triangles
                </p>
              </div>
            </div>
          ...
          <div className="text-xs font-mono text-[var(--muted-foreground)]">
            {isConverting ? (
              <span className="flex items-center gap-2">
                ...
                {progressText || "Computing 3D geometry buffers..."}
              </span>
            ) : (
              <span>100% Client-Side WebGL 3D Mesh Engine</span>
            )}
          </div>
