...
export function ArchiveDropzone({
  ...
  const handleDrop = (e: DragEvent<HTMLDivElement>) => {
    ...
    if (e.dataTransfer.files && e.dataTransfer.files.length > 0) {
      ...
      if (isArchiveFile(file) || file.name.endsWith(".zip") || file.name.endsWith(".tar") || file.name.endsWith(".gz") || file.name.endsWith(".tgz") || file.name.endsWith(".7z") || file.name.endsWith(".rar")) {
        onFileSelect(file);
      }
    }
  };
  ...
  return (
    ...
    >
      <input
        ref={inputRef}
        type="file"
        accept=".zip,.tar,.gz,.tgz,.7z,.rar,.bz2,.xz,.iso,.dmg,.cab,.jar,.apk,.deb,.rpm"
