...
export function ArchiveFileTree({ entries }: ArchiveFileTreeProps) {
  return (
    <div className="space-y-1.5">
      ...
      <div className="max-h-40 overflow-y-auto rounded border border-(--border) bg-(--foreground)/[0.01] p-2 space-y-1">
        {entries.map((entry) => (
          <div
            key={entry.path}
            className="flex items-center justify-between py-1 px-2 rounded hover:bg-(--foreground)/5 text-xs font-mono text-(--foreground)"
          >
