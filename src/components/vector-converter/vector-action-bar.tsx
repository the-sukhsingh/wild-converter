...
import { type VectorFormat, VECTOR_FORMATS } from "@/lib/vector-format-utils";
import { formatFileSize } from "@/lib/format-utils";

interface VectorActionBarProps {
  targetFormat: VectorFormat;
        ...
        <div className="w-full flex flex-col gap-1 py-1">
          <div className="flex items-center justify-between text-xs font-mono text-[var(--muted-foreground)]">
            <span className="flex items-center gap-1.5 truncate max-w-md">
              ...
              <span>{progressText || "Compiling vector paths..."}</span>
            </span>
          ...
          >
            {isConverting ? (
              <>
                ...
                <span>{progress !== undefined ? `${Math.round(progress)}%` : "Compiling…"}</span>
              </>
            ) : (
              <>
