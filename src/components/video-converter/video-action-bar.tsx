...
import type { VideoConversionResult } from "@/lib/video-converter";
    ...
    <div className="w-full pt-4 border-t border-[var(--border)] space-y-3">
      {result ? (
        <div className="space-y-3">
          {/* Preview for converted video / GIF / audio */}
          <div className="p-3 bg-[var(--foreground)]/[0.03] border border-[var(--border)] rounded flex flex-col sm:flex-row items-center justify-between gap-3">
            ...
            {result.isAnimation ? (
              ...
              <img
                ...
                className="h-16 max-w-full object-contain rounded border border-[var(--border)]"
              />
            ) : result.isAudio ? (
              <audio
                controls
              ...
              <video
                ...
                playsInline
                src={result.url}
                className="h-16 max-w-full rounded border border-[var(--border)]"
              />
            )}
          </div>
          ...
          <div className="text-xs font-mono text-[var(--muted-foreground)]">
            {isConverting ? (
              <span className="flex items-center gap-2">
                ...
                {progressText || "Transcoding video stream..."}
              </span>
            ) : (
              <span>100% Client-Side Canvas & MediaStream WASM Engine</span>
            )}
          </div>
