...
type TabCategory = "all" | "popular" | "animation" | "modern" | "mobile" | "audio-extract" | "lossless-preset";
...
export function VideoFormatSelector({
  ...
  const allFormats = Object.values(VIDEO_FORMATS);
    ...
    <div className="space-y-3">
      <div className="flex items-center justify-between">
        ...
        <span className="text-xs font-mono text-[var(--muted-foreground)]">
          {VIDEO_FORMATS[selectedFormat]?.description}
        </span>
      </div>
      ...
      <div className="flex items-center gap-1 overflow-x-auto pb-1 border-b border-[var(--border)] text-xs font-mono scrollbar-none">
        {(
          [
            ...
            { id: "animation", label: "GIF Animation" },
            { id: "audio-extract", label: "Audio Extract" },
            { id: "modern", label: "Modern / WebM" },
            { id: "mobile", label: "Mobile (3GP)" },
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-1.5 max-h-48 overflow-y-auto pr-1">
        {filteredFormats.map((fmt: VideoFormatInfo) => {
