...
interface CodeFormatSelectorProps {
  ...
  onSelectFormat: (format: CodeFormat) => void;
  disabled?: boolean;
}

type TabCategory = "all" | "web" | "systems" | "data" | "scripts" | "classic" | "lossless-preset";

export function CodeFormatSelector({
  selectedFormat,
  onSelectFormat,
  disabled = false,
}: CodeFormatSelectorProps) {
  const [activeTab, setActiveTab] = useState<TabCategory>("web");

  const allFormats = Object.values(CODE_FORMATS);
    ...
    <div className="space-y-3">
      <div className="flex items-center justify-between">
        ...
        <span className="text-xs font-mono text-[var(--muted-foreground)]">
          {CODE_FORMATS[selectedFormat]?.description}
        </span>
      </div>
      ...
      <div className="flex items-center gap-1 overflow-x-auto pb-1 border-b border-[var(--border)] text-xs font-mono scrollbar-none">
        {(
          [
            { id: "web", label: "Web / Frontend (TS, JS, HTML, CSS)" },
            { id: "systems", label: "Systems (Rust, Go, C++, C#, Java)" },
            { id: "data", label: "Data / Database (JSON, YAML, SQL)" },
            { id: "scripts", label: "Scripts (Python, Ruby, Lua, R)" },
            { id: "classic", label: "Classic (COBOL, Fortran, Ada)" },
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-1.5 max-h-48 overflow-y-auto pr-1">
        {filteredFormats.map((fmt: CodeFormatInfo) => {
