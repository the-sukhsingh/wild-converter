...
import type { CodeConversionResult } from "@/lib/code-converter";

interface CodeActionBarProps {
  isConverting: boolean;
  progressText?: string;
  result: CodeConversionResult | null;
  onConvert: () => void;
  onReset: () => void;
  disabled?: boolean;
}
    ...
    <div className="w-full pt-4 border-t border-[var(--border)] space-y-3">
      {result ? (
        <div className="space-y-3">
          ...
          <div className="p-3 bg-[var(--foreground)]/[0.03] border border-[var(--border)] rounded space-y-2">
              ...
              <div className="flex items-center gap-2 min-w-0">
                ...
                <div className="min-w-0">
                  ...
                  <p className="text-[11px] font-mono text-[var(--muted-foreground)]">
                    {formatBytes(result.fileSizeBytes)} • {result.lineCount.toLocaleString()} lines
                  </p>
                </div>
              </div>
            ...
            <pre className="max-h-48 overflow-auto p-2.5 rounded bg-[var(--foreground)]/[0.02] border border-[var(--border)]/60 text-xs font-mono text-[var(--foreground)] select-all">
              <code>{result.formattedCode}</code>
            </pre>
          </div>
          ...
          <div className="text-xs font-mono text-[var(--muted-foreground)]">
            {isConverting ? (
              <span className="flex items-center gap-2">
                ...
                {progressText || "Formatting code tokens..."}
              </span>
            ) : (
              <span>100% Client-Side AST Tokenizer & Transpiler Engine</span>
            )}
          </div>
