...
export function CodeDropzone({
  ...
  const handleDrop = (e: DragEvent<HTMLDivElement>) => {
    ...
    if (e.dataTransfer.files && e.dataTransfer.files.length > 0) {
      ...
      if (isCodeFile(file) || file.type.includes("text") || file.name.includes(".")) {
        onFileSelect(file);
      }
    }
  };
    ...
    >
      ...
      <div className="space-y-1.5 mb-3">
        <p className="text-sm font-medium text-[var(--foreground)]">
          Drop your code, script, or markup file here, or{" "}
          <span className="underline underline-offset-4">browse</span>
        </p>
        <p className="text-xs text-[var(--muted-foreground)]">
          Supports TypeScript, JS, Python, Rust, Go, C++, HTML, CSS, SQL, JSON, YAML & 30+ languages
        </p>
      </div>

      <div className="flex flex-wrap items-center justify-center gap-1.5 max-w-md pt-2">
        {["TypeScript", "JavaScript", "Python", "Rust", "Go", "HTML", "CSS", "JSON", "YAML", "SQL"].map(
          (ext) => (
            <span
              key={ext}
